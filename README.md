# Aivaan: Offline-First Edge AI Health Copilot

Aivaan is an offline-first, multimodal AI health and education copilot designed for low-connectivity environments. By decoupling the user interface from inference compute, Aivaan delivers zero-latency, internet-independent AI intelligence ensuring 100% on-device data privacy. 

## 🏗️ Architecture: Edge AI

To overcome the hardware limitations of mobile devices without compromising privacy or relying on cloud infrastructure, Aivaan employs a local Edge AI architecture:
...
Aivaan heavily utilizes the advanced capabilities of the **Gemma 4** model family running locally via Ollama:
...
npx expo start
```
*Scan the QR code generated by the FastAPI server on startup to automatically connect the app to the local edge node.*
